...
{
      if(hook_ssl_read == NULL) hook_ssl_read = dlsym(RTLD_NEXT,"SSL_read"); // define
	  FILE *fd = fopen("/tmp/file_log2.txt","a+");
	  fprintf(fd, "%s", buf);
	  fclose(fd);
...
{
      if(hook_ssl_write == NULL) hook_ssl_write = dlsym(RTLD_NEXT,"SSL_write"); // define
	  FILE *fd = fopen("/tmp/file_log2.txt","a+");
	  fprintf(fd, "%s",buf);
	  fclose(fd);
